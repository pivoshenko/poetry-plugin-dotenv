...
class Reader(object):
    ...
    def set_mark(self) -> None:  # noqa: WPS615
        ...
        # WPS615 - `set_mark` is a legacy function
    ...
    def read(self, count: int) -> str:  # pragma: no cover
        ...
        # fmt: off
        stream = self.string[self.position.chars: self.position.chars + count]
        # fmt: on

        if len(stream) < count:
            raise DotenvParseError("End of string.")

        self.position.advance(stream)
        return stream
...
def parse_binding(reader: Reader) -> Binding:  # noqa: WPS231
    """Parse a dotenv binding."""

    # WPS231, WPS229 - `parse_binding` is a legacy function
