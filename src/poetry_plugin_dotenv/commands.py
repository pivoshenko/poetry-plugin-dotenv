...
class ActivateCommand(EnvCommand):
    ...
    def handle(self) -> int:  # pragma: no cover
        ...
        if not shell:
            ...
            if pwsh_present:
                ...
                if ps_exe:
                    # -NoExit keeps the session interactive after activation
    ...
    def _get_venv_activation_command(self, venv: Env) -> str | None:  # pragma: no cover
        ...
        if os.name == "nt" or not shell:
            ...
            if ps1.exists():
                # Dot-source the script so its variables reach the session
                return f"& '{ps1}'"

            bat = venv.path / "Scripts" / "activate.bat"
            if bat.exists():
                # Bare path; the caller invokes it as cmd.exe /k <bat>
                return str(bat)
